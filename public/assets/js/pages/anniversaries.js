...
document.getElementById('pageBody').innerHTML = `
  ...
  <div class="modal fade" id="wishModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    ...
    <div class="modal-body" id="anniversaryWishForm">
    ...
    <div class="modal-footer"><button class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button class="btn btn-primary" onclick="sendAnniversaryWishes(this)">Send</button></div>
  </div></div></div>`;
...
(async () => {
  ...
  document.getElementById('todayGrid').innerHTML = todayAnniversaries.length ? todayAnniversaries.map(c => `
    <div class="col-md-4"><div class="card"><div class="card-body text-center">
      ...
      <button class="btn btn-primary btn-sm mt-2" onclick="sendAnniversaryWish('${c._id}', this)"><i class="bi bi-heart"></i> Send Wish</button>
    </div></div></div>`).join('') : '<div class="col-12 empty-state"><i class="bi bi-heart d-block"></i>No anniversaries today</div>';
  document.getElementById('upcomingBody').innerHTML = upcoming.map(c => `<tr><td>${c.firstName} ${c.lastName}</td><td>${c.designation || '-'}</td><td>${RMS.utils.formatDate(c.anniversary)}</td><td>${c.city || '-'}</td><td><button type="button" class="btn btn-sm btn-outline-primary" onclick="sendAnniversaryWish('${c._id}', this)" aria-label="Send anniversary wish"><i class="bi bi-send"></i></button></td></tr>`).join('') || '<tr><td colspan="5" class="text-center text-secondary">None upcoming</td></tr>';
...
window.sendAnniversaryWishes = async (button) => {
  const contactIds = todayAnniversaries.map(c => c._id);
  if (!contactIds.length) {
    return RMS.mutations.showValidationError('#anniversaryWishForm', 'No anniversaries today');
  }
